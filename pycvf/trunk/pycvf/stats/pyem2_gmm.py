...
try:
    ...
    import sckits.learn.machine.em2 as em2  
...
class Em2Model:
   ...
   def __init__(self,ndim,k=9,mode=None):
      ...
      self.lgm = em2.gmm.Parameters(ndim, k, "diag") ## ndim , k: mixture, "full" covariance as opposed to diag...
      #self.gmm = em2.gmm.Parameters(self.lgm, "kmean")
      self.em=em2.gmm.EM()
   def train(self,positive_training_set,negative_training_set=None, maxiter = 30):

      trained=False
      while not trained: 
        try:
          self.em.train(numpy.array(positive_training_set), self.lgm, maxiter=maxiter)
          trained=True
   ...
   def test(self,data,log=True):
      from em2.gmm import logresp as plogresp
      #print data
      return plogresp( numpy.array(data).astype('float64'),self.lgm.w,self.lgm.mu, self.lgm.va)
